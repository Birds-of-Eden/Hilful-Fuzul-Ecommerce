...
const API_BASE = '/api/newsletter'; // Assuming the base API path
...
export default function NewsletterManagement() {
  ...
  const [deletingId, setDeletingId] = useState<string | null>(null); // New state for ID to delete
  ...
  // Use a simple mock toast for the environment, as the real one is likely external
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast("Success", `Newsletter ${editingNewsletter ? "updated" : "created"} successfully`);
        fetchNewsletters();
  ...
  const handleSend = async (id: string) => {
    try {
      ...
      showToast("Error", error.message || "Failed to send newsletter", "destructive");
    } finally {
      setSendingId(null);
    }
  };

  // --- Deletion Handler Update ---
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="max-w-sm mx-4 bg-white/95 backdrop-blur-sm border border-[#D1D8BE] rounded-2xl shadow-2xl p-6">
                <DialogHeader className="border-b border-[#D1D8BE] pb-3 mb-4">
                    <DialogTitle className="text-xl font-semibold text-[#C0704D]">Confirm Deletion</DialogTitle>
                </DialogHeader>
